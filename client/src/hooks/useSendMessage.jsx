...
const useSendMessage = (setMessageEmpty) => {
  ...
  const sendMessage = (messageData) => {
    ...
    if (!userId) {
      console.error("❌ Cannot send message: userId is not set");
      return;
    }
    
    if (!chatRoomId) {
      console.error("❌ Cannot send message: chatRoomId is not set");
      return;
    }
    
    console.log("📤 Sending message...", { userId, chatRoomId, messageMode });
    ...
    // Emit message event
    const finalChatRoomId = messageData?.callType || messageData?.botId
    ...
    console.log("📤 Emitting user:message event:", payload);
    
    socketEmit("user:message", payload);
