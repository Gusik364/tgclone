...
const chatSlice = createSlice({
  ...
  reducers: {
    ...
    deleteMessage: (state, { payload: { messageId, day, deleteFor } }) => {
      const currentChatRoom = current(state.currentChatRoom);
      const chatRoomId = currentChatRoom._id;
      
      if (!chatRoomId || !state.chatHistory[chatRoomId]) return;

      const messageHistory = state.chatHistory[chatRoomId].messageHistory;
      const dayMessagesIndex = messageHistory.findIndex(
        (dayMessage) => dayMessage.day === day
      );
      
      if (dayMessagesIndex === -1) return;
      ...
      if (deleteFor === "everyone") {
        ...
        state.chatHistory[chatRoomId].messageHistory[dayMessagesIndex].messages.splice(messageIndex, 1);
        
        // If no messages left in this day, remove the day
        if (state.chatHistory[chatRoomId].messageHistory[dayMessagesIndex].messages.length === 0) {
          state.chatHistory[chatRoomId].messageHistory.splice(dayMessagesIndex, 1);
        }
      } else {
        // Mark as deleted for me
        state.chatHistory[chatRoomId].messageHistory[dayMessagesIndex].messages[messageIndex].isDeleted = true;
      }

      // Update currentChatRoom
      state.currentChatRoom.messageHistory = current(state.chatHistory[chatRoomId].messageHistory);
    },
