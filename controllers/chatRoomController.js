  ...
  let chatRoomSummary = await Promise.all(
    user.chatRooms.map(async (chatRoomId) => {
      ...
      const chatRoom = await ChatRoom.findById(chatRoomId).populate({
        path: "members",
        select: "id username avatar bio status isVerified isBot isScam scamReason nftUsernames anonymousNumber",
      });
...
exports.getMessageFromChatRoom = async ({ chatRoomId, messageId, day }) => {
  ...
  if (!chatRoom || !chatRoom.messageHistory || !chatRoom.messageHistory.length) return {};
...
// Delete message
exports.deleteMessage = async ({ chatRoomId, messageId, day, deleteFor, userId }) => {
  const chatRoom = await ChatRoom.findById(chatRoomId);
  if (!chatRoom) return { success: false };

  const dayMessage = chatRoom.messageHistory.find(
    (dm) => dm.day === day
  );
  if (!dayMessage) return { success: false };

  const messageIndex = dayMessage.messages.findIndex(
    (m) => m._id.toString() === messageId.toString()
  );
  if (messageIndex === -1) return { success: false };

  const message = dayMessage.messages[messageIndex];
  ...
  if (deleteFor === "everyone") {
    ...
    dayMessage.messages.splice(messageIndex, 1);
    
    // If no messages left in this day, remove the day
    if (dayMessage.messages.length === 0) {
      chatRoom.messageHistory = chatRoom.messageHistory.filter(
        (dm) => dm.day !== day
      );
    }
  } else {
